...
<template>
 <el-drawer v-model="visible" :title="(row.version?'编辑':'新增')+label" size="720px" class="course-editor-drawer" :close-on-click-modal="false" :before-close="close">
  <el-form v-if="visible" label-position="top" class="editor-form">
   ...
   <el-alert v-if="parent.kind==='knowledge'" class="presentation-notice" :title="row.payload.type==='article'?'发布后，此图文直接作为知识点正文展示':'发布后，播放器直接展示在知识点页面'" :description="row.payload.type==='article'?'前端不再重复展示知识点原文；原文仍保留，下架或删除全部图文配套课后恢复。课程阅读仍按所属考试权益控制。多篇已发布图文按创建顺序展示。':'无需先点击课程卡片展开。用户点击播放后开始学习，不自动播放；知识点原文继续保留在播放器下方。'" type="info" :closable="false" show-icon/>
   <el-form-item v-if="row.payload.type==='article'" :label="parent.kind==='knowledge'?'知识点展示正文':'正文'"><RichEditor :key="row.id" v-model="row.payload.document" :plain-text="row.payload.content" :exam-id="row.exam_id" :content-id="row.id" :allow-media="false" :allow-images="true" :allow-handouts="false" @busy="busy.document=$event"/></el-form-item>
   <template v-else>
   ...
   <el-form-item label="封面图（建议 16:9，公开展示）"><CourseAssetPicker :key="row.id+'cover'" v-model="row.payload.posterAssetId" kind="image" label="封面图" :exam-id="row.exam_id" :content-id="row.id" @busy="busy.poster=$event"/></el-form-item>
...
<style scoped>.presentation-notice{margin-bottom:20px}.course-footer{display:flex;justify-content:space-between;align-items:center;gap:12px}.course-footer>div{margin-left:auto;display:flex;gap:10px}.course-footer .el-button+.el-button{margin:0}.course-footer svg{margin-right:5px}.editor-form :deep(.el-select){width:100%}</style>
