...
async function add(){
  ...
  if(props.modelValue.length>=20){error.value='每个知识点最多添加20份讲义';return}
...
<template>
  <section class="knowledge-handouts" aria-label="知识点配套讲义">
    <p class="handout-hint">前端独立显示下载按钮，用户下载后可在“我的讲义”中找到。</p>
    ...
    <p v-else class="handout-empty">暂未添加讲义</p>
    ...
    <template v-if="source==='upload'"><input ref="input" class="handout-file-input" type="file" accept=".pdf,.docx,.pptx" aria-label="选择讲义文件" :disabled="busy" @change="choose" /><p class="handout-hint">支持 PDF、Word（DOCX）、PPTX，每份不超过200MB。</p></template>
