...
const filtered=computed(()=>points.value.filter(p=>[subject.value,chapter.value,section.value].every(id=>!id||p.ancestorIds.includes(id))&&(!keyword.value.trim()||(p.title+' '+p.path).toLocaleLowerCase().includes(keyword.value.trim().toLocaleLowerCase()))))
