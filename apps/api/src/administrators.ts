...
administrators.get('/', async (req, res) => {
  ...
  const where = "account_kind='admin' AND (phone ILIKE $1 OR nickname ILIKE $1)"
...
administrators.post('/', async (req, res) => {
  ...
  const result = await transaction(async c => {
    const result = (await c.query(`INSERT INTO users(id,phone,nickname,role,password_hash,invite_code,account_kind,is_test_data) VALUES($1,$2,$3,'superadmin',$4,$5,'admin',$6) RETURNING ${fields}`, [id(), b.phone, b.nickname, passwordHash(b.password), String(randomInt(10000000,99999999)), process.env.APP_MODE !== 'production'])).rows[0]
...
administrators.patch('/:id', async (req, res) => {
  ...
  const result = await transaction(async c => {
    ...
    const old = (await c.query("SELECT id,nickname,enabled,role FROM users WHERE id=$1 AND account_kind='admin'", [req.params.id])).rows[0]
    if (!old) fail(404, '管理员不存在')
    if (!b.enabled) {
      if (old.id === res.locals.user.id) fail(409, '不能停用当前登录的管理员')
      const active = (await c.query("SELECT count(*)::int AS n FROM users WHERE account_kind='admin' AND role='superadmin' AND enabled=true AND id<>$1", [old.id])).rows[0].n
...
administrators.post('/:id/password', async (req, res) => {
  ...
  await transaction(async c => {
    const result = await c.query("UPDATE users SET password_hash=$2,updated_at=now() WHERE id=$1 AND account_kind='admin' RETURNING id", [req.params.id, passwordHash(b.password)])
