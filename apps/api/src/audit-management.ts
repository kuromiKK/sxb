...
auditManagement.get('/records',async(req,res)=>{
 ...
 const where="($1='' OR a.action=$1) AND (coalesce(u.nickname,'系统自动处理') ILIKE $2 OR coalesce(u.phone,'') ILIKE $2) AND coalesce(a.target_id,'') ILIKE $3 AND ($4::timestamptz IS NULL OR a.created_at >= $4) AND ($5::timestamptz IS NULL OR a.created_at <= $5)"
