...
import { bulkContent } from './bulk-content.ts'
import { importJobs,contentOrigins } from './import-jobs.ts'
...
api.use('/admin/import-jobs',importJobs)
api.use('/admin/bulk-content',bulkContent)
api.get('/admin/content-origins/:id',async(req,res)=>res.json(await contentOrigins(req.params.id)))
...
api.get('/admin/content',async(req,res)=>{
  ...
  const where=`WHERE NOT (payload ? 'deletedAt') AND ($1::text IS NULL OR kind=$1) AND (title ILIKE $2 OR id ILIKE $2) AND ($3::text IS NULL OR exam_id=$3) AND ($4::text IS NULL OR parent_id=$4)
...
api.get('/admin/users',async(req,res)=>{
  ...
  res.json((await db.query(`SELECT u.id,u.phone,u.nickname,u.role,u.invite_code,u.inviter_id,u.created_at,u.is_test_data,u.is_developer,i.nickname AS inviter,(SELECT min(paid_at) FROM orders o WHERE o.user_id=u.id) AS first_paid_at FROM users u LEFT JOIN users i ON i.id=u.inviter_id WHERE u.account_kind='student' AND (u.phone ILIKE $1 OR u.nickname ILIKE $1 OR u.id ILIKE $1) AND ($2::boolean IS NULL OR u.is_developer=$2) ORDER BY u.created_at DESC LIMIT 200`,[`%${search}%`,developer])).rows)
})
