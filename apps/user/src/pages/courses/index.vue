...
<template>
  <view class="courses-page page safe-top">
    ...
    <view class="subject-chips"><button v-for="subject in knowledgeSubjects" :key="subject.id" class="subject-chip" :aria-pressed="selectedSubjectId === subject.id" :class="{ active: selectedSubjectId === subject.id }" @tap="selectSubject(subject.id)"><uni-icons :type="subject.id === 'ability' ? 'map' : 'list'" size="16" :color="selectedSubjectId === subject.id ? '#fff' : subject.id === 'ability' ? '#3569e8' : '#e98a3a'" /><text>{{ subject.name }}</text></button></view>
