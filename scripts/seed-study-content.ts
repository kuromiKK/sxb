...
try {
  ...
  await transaction(async c=>{
    await c.query(`INSERT INTO content(id,exam_id,kind,parent_id,title,status,payload,source,is_test_data) SELECT $1,$2,'knowledge','ability-section-1-1','【测试内容】知识点图文与音视频示例','published',$3,'study_content_test',true WHERE NOT EXISTS(SELECT 1 FROM content WHERE id=$1)`,[point,exam,JSON.stringify({stars:3,no:999,isKnowledgeCourse:true,content:'【测试内容】服务对象在充分知情的基础上参与选择。',document})])
    await c.query(`INSERT INTO content(id,exam_id,kind,title,status,payload,source,is_test_data) SELECT $1,$2,'cheatsheet','【测试内容】考前复盘清单','published',$3,'study_content_test',true WHERE NOT EXISTS(SELECT 1 FROM content WHERE id=$1)`,[sheet,exam,JSON.stringify({intro:'概念辨析、错题回看与考前节奏。仅用于本机流程验证。',opensAt:new Date(Date.now()-86400000).toISOString(),closesAt:new Date(Date.now()+30*86400000).toISOString(),document:sheetDoc})])
